...
1. Add this add-on repository to your Home Assistant instance.
1. Search for "Healthchecks" in the add-on store and install it.
...
1. Start the add-on and open the panel in the sidebar.

## Two ways in

The add-on serves the same instance twice:
...
Creates the first account the first time the add-on starts. Both are needed.
...
the add-on's own storage, and is included in Home Assistant backups. It is a
...
to exist already; the add-on creates the schema but not the database.
...
port in the add-on's network configuration too.
...
Controls both the add-on's own startup logging and how much Healthchecks writes
to the add-on log.

## Notes on how this add-on is put together
...
serve both the ingress prefix and the root prefix correctly. The add-on runs
...
Open an issue on [the add-on repository][issues]. For questions about
