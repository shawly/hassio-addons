...
Everything in an add-on's directory in this repository is **generated**. The
[Repository Updater](https://github.com/hassio-addons/repository-updater) copies it in
from the add-on's own repository on every release, so changes made here are overwritten
on the next update.

To change an add-on, open a pull request against its repository. `.apps.yml` in the root
